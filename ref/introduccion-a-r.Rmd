...
Tal como indiqué arriba, los mapas generados con la función `plot` son muy mejorables. En R no es fácil cumplir con los estándares propios de un mapa cartográficamente apropiado. Por ejemplo, eliminar el "efecto isla" requeriría retoques manuales, y rotular objetos de referencia en el mapa requeriría también acciones manuales. Sin embargo, no podemos olvidar que en R lo que más nos interesa es explorar e interpretar patrones, por lo que la simbología y está orientada a actuar sobre la o las variables de nuestro interés. En tal sentido, los paquetes `ggplot2` (parte de `tidyverse`) y `tmap` ofrecen herramientas versátiles para conseguir mapas informativos, al menos en lo que respecta a la variable de interés.
